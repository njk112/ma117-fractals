...
/*
 * PROJECT II: NewtonFractal.java   --- Naglis
 *
...
class NewtonFractal {
    ...
    public void printRoots() {
        // You need to fill in this function.
        for (Complex rot : roots) System.out.println(rot.toString());
    }
    ...
    public int findRoot(Complex root) {
        // You need to fill in this function.
        int i =0;
        for (Complex rot : roots) {
            if (rot.add(root.minus()).abs() < Newton.TOL) return i;
            i+=1;
        }
        return -1;
    }
    ...
    public Complex pixelToComplex(int i, int j) {
        ...
        double x = origin.getReal()  + i*dz+dz/2;
        double y = origin.getImag() - j*dz-dz/2;
   ...
   public void createFractal(boolean colorIterations) {
        ...
        // Iterate through all pixels with j,k coordinates using nested loop
        for (int j = 0; j < NUMPIXELS; j++) {
        	for (int k = 0; k < NUMPIXELS ; k++) {
        		// Use Newton-Raphson method
        		iterator.iterate(pixelToComplex(k,j));
        		// Add root to the list if it hasn't been added before
        		if (findRoot(iterator.getRoot()) == -1){ roots.add(iterator.getRoot());}
        		// Color pixel if error is 0, otherwise leave black
        		if (this.iterator.getError() == 0) colorPixel(k,j,findRoot(iterator.getRoot()),iterator.getNumIterations());
        	}
        }
    }
    ...
    public static void main(String[] args) {
        ...
        NewtonFractal f = new NewtonFractal(p, new Complex(-1.0, 1.0), 2);
